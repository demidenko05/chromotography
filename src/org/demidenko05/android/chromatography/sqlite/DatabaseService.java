...
public class DatabaseService {
	/**
	 * multi-thread ready
	 * works when synchronized or not
	 */
	private SQLiteDatabase db;
	private ChromSqliteHelper sqliteHelper;
	...
	public void init(Context context) {
		sqliteHelper = new ChromSqliteHelper(context);
		db = sqliteHelper.getWritableDatabase();
		OrmServicesFactory ormServicesFactory = OrmServicesFactory.getInstance();
		if(ormServicesFactory.getOrmServices() == null)
			ormServicesFactory.init(db);

	}
		
	public void fillSeriesAnalyteList(List<Analyte> listSolvents, long idSeriesHead) {
	...
	public void fillSeriesSolventsList(List<SeriesSolvents> listSolvents,  long idSeriesHead) {
	...
	public void fillSeriesList(List<SeriesHead> list,  String selection) {
	...
	public String[] getTitles(Map<String, SeriesHead> seriesToShow) throws SeriesNotSameDurationException, IncompleteDataException {//analytes
		...
		for(Entry<String, SeriesHead> entry : seriesToShow.entrySet()) {
			synchronized (db) {
				cursor = db.rawQuery(queryStr, new String[]{Long.toString(entry.getValue().getId())});
			}
			if(cursor.moveToFirst()) {
	...
	public double[][] getYAxisDataList(Map<String, SeriesHead> seriesToShow) {
		int size = 0;
		for(Entry<String, SeriesHead> entry : seriesToShow.entrySet()) {
		...
		for(Entry<String, SeriesHead> entry : seriesToShow.entrySet()) {
			for(SeriesBody serBody : entry.getValue().getSeriesBody()) {
				synchronized (db) {
					cursor = db.query(OrmServicesFactory.getInstance().getOrmService(SeriesBody.class).getTableName(),
	...
	public double[] getXAxisData(Map<String, SeriesHead> seriesToShow) {
	...
	public void deleteSeriesBody(long idSeries, long idAnalyte) {
		Cursor cursor;
		synchronized (db) {
			cursor = db.query(OrmServicesFactory.getInstance().getOrmService(SeriesBody.class).getTableName(),
		...
		if(cursor.moveToFirst()) {
			while(!cursor.isAfterLast()) {
				...
				OrmServicesFactory.getInstance().getOrmService(SeriesBody.class).delete(serBody);
				cursor.moveToNext();
			}
		}
	}
